...
int main ()
{
	char c [10]; // Declaramos longitud maxima de caracteres siempre entre []
	
	printf ("Ingrese su nombre: "); // Pedimos al usuario que ingrese su nombre
	gets(c); // Almacenamos lo que ingreso en la variable C
	printf("\nEl nombre ingresado es: "); 
	puts(c); // Mostramos C a traves de un PUTS, siempre entre ()
	printf ("Mi nombre es %s", c); // Mostramos C sin necesidad de un PUTS
	return 0 ;
}
