...
int main() {
	...
	for (i = 0; i < ELEM; i++) {
		...
		printf("Ingrese el número de teléfono del amigo %d: ", i + 1);
	...
	// Ordenamiento de la lista con la función BUBBLE SORT
	...
	// Búsqueda de un amigo por su DNI (binaria)
	...
	// Búsqueda binaria de un amigo por su DNI en un arreglo ordenado en orden descendente
	while (inicio <= fin) {
		medio = (inicio + fin) / 2; // Calcula el índice del medio del rango actual
		
		if (amigo[medio].dni == b) {
			// Si el DNI en la posición media coincide con el buscado
			encontrado = 1; // Marca como encontrado
			break;          // Sale del bucle
		} else if (amigo[medio].dni < b) {
			// Como el arreglo está en orden descendente,
	...
	if (encontrado) {
		// Si se encontró el amigo con ese DNI
		printf("DNI encontrado: %d\n", amigo[medio].dni);
		printf("Nombre: %s %s | Teléfono: %s | Edad: %d\n",
			   amigo[medio].nombre, amigo[medio].apellido, amigo[medio].telefono, amigo[medio].edad);
	} else {
		// Si no se encontró ningún amigo con ese DNI
	...
	printf("\nNúmero de amigos mayores a %d años: %d\n", edadReferencia, mayores);
	printf("Número de amigos menores a %d años: %d\n", edadReferencia, menores);
	if (iguales > 0){
		printf("Número de amigos con %d años: %d\n", edadReferencia, iguales);
	}
