...
int main() {
	int arreglo[10];        // Arreglo donde se guardar�n los 10 n�meros ingresados
	int matriz[3][3];       // Matriz 3x3 para almacenar los primeros 9 n�meros del arreglo
	
	int i = 0;
	int errores_totales = 0;        // Contador de errores totales al ingresar n�meros
	int errores_consecutivos = 0;   // Contador de errores consecutivos
	
	// Solicita al usuario ingresar 10 n�meros v�lidos entre 1 y 100
	...
	while (i < 10) {
		...
		// Si el n�mero est� dentro del rango v�lido, se guarda
		if (num >= 1 && num <= 10) {
			arreglo[i] = num;
			i++;
			...
			// Si se ingresan 3 errores consecutivos, el cuarto valor ser� 1 autom�tico
			if (errores_consecutivos >= 4) {
				printf("Se han ingresado 3 valores incorrectos consecutivos. Asignando 1 autom�ticamente.\n");
				arreglo[i] = 1;
				i++;
				errores_consecutivos = 0;  // Reinicia errores consecutivos
			} else {
	...
	// Muestra el contenido del arreglo, un n�mero por l�nea
	printf("\nArreglo cargado:\n");
	for (int j = 0; j < 10; ++j) {
		printf("%d\n", arreglo[j]);
	}
	
	// Muestra la cantidad total de errores cometidos al ingresar n�meros
	...
	// Crea una cadena de caracteres con letras que representan los n�meros del arreglo
	...
	for (int j = 0; j < 10; ++j) {
		...
		default: cadena[j] = '_'; break; // Otros n�meros se representan con gui�n bajo
		}
	}
	...
	if (total_vocales > 0) {
		...
		// Si no hay ninguna vocal v�lida, se informa que la cadena est� vac�a
		printf("\nLa cadena est� vac�a (sin vocales v�lidas).\n");
	}
